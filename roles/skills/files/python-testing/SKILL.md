...
- Still mock external services: databases, external APIs, message queues, cloud services, etc.
...
```python
@patch("myapp.module.submodule.AppConfig")
@patch("myapp.module.submodule.get_secret")
def test_read_vault_secrets(self, mock_get_secret, mock_app_config):
    # Decorators apply bottom-up; args are passed left-to-right (innermost first)
    mock_config = MagicMock()
    mock_config.get.return_value = "service.example.com"
    mock_app_config.return_value = mock_config

    mock_get_secret.side_effect = lambda path: {
        "service_a": {"username": "vault_user", "api_token": "vault_token"},
        "services": {"service.example.com": {"ssh_user": "git_user", "ssh_key": "git_key"}},
        "service_b": {"api_token": "service_b_token"},
    }[path]

    result = Secrets()

    mock_get_secret.assert_any_call("service_a")
    mock_config.get.assert_called_once_with("service", "url")
    self.assertEqual(result.service_a_username.get_secret_value(), "vault_user")
```
...
from myapp.models import DataItem

mock_obj = Mock(spec=DataItem)
mock_obj.name = "item-a"
mock_obj.meets_condition.return_value = True
```
...
from myapp.models.storage import StorageBackend


class TestAppConfigIntegration(unittest.TestCase):
    """Component tests for AppConfig integration with other modules."""

    def setUp(self):
        self.config = AppConfig()

    def test_storage_type_integration(self):
        """Test that storage config integrates with the storage model."""
        backend_type = self.config.get("storage", "type")
        self.assertEqual(backend_type, StorageBackend)
        instance = backend_type()
        self.assertIsInstance(instance, StorageBackend)

    def test_multiple_instances_are_independent(self):
        """Test that two AppConfig instances don't share state."""
        config2 = AppConfig()
        self.assertIsNot(self.config, config2)
        self.assertEqual(self.config.get("storage"), config2.get("storage"))
```
...
4. **Mock all external dependencies** in unit tests — network, external APIs, databases, filesystem.
